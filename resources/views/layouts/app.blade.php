                    ...
                    <div class="collapse navbar-collapse" id="navbarNav">
                        <ul class="navbar-nav ms-auto">
                            ...
                            @if(auth()->user() && auth()->user()->hasRole('admin'))
                                <li class="nav-item">
                                    <a class="nav-link {{ Route::currentRouteName() == 'admin.portal' ? 'active' : '' }}"
                                        aria-current="page" href="{{ route('admin.portal') }}">Portal Admin</a>
                                </li>
                            @endif
                            <!-- Enlace a la página de roles (solo visible para administradores) -->
                            {{-- @if(auth()->user() && auth()->user()->hasRole('admin'))
                                <li class="nav-item">
                                    <a class="nav-link {{ Route::currentRouteName() == 'assign.role' ? 'active' : '' }}"
                                        href="{{ route('assign.role') }}">Asignar Roles</a>
                                </li>
                            @endif                          <!-- Enlace para gestión de estudiantes (visible para admin, director y maestro) --> --}}
                            @if(auth()->user() && (auth()->user()->hasRole('admin') || auth()->user()->hasRole('director') || auth()->user()->hasRole('maestro')))
                                <li class="nav-item">
                                    <a class="nav-link {{ Route::currentRouteName() == 'students.show' ? 'active' : '' }}"
                                        href="{{ route('students.show') }}">Gestión de Estudiantes</a>
                                </li>
                            @endif
                            ...
                            @if(auth()->user() && (auth()->user()->hasRole('admin') || auth()->user()->hasRole('director') || auth()->user()->hasRole('supervisor')))
                                <li class="nav-item">
                                    <a class="nav-link {{ Route::currentRouteName() == 'students.list' ? 'active' : '' }}"
                                        href="{{ route('students.list') }}">Lista de Estudiantes</a>
                                </li>
                            @endif
